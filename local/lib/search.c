...
static void traceptrs(SynsetPtr synptr, int ptrtyp, int dbase, int depth) {
	...
	for (i = 0; i < synptr->ptrcount; i++) {
		if ((ptrtyp == HYPERPTR
				&& (synptr->ptrtyp[i] == HYPERPTR
						|| synptr->ptrtyp[i] == INSTANCE))
				|| (ptrtyp == HYPOPTR
						&& (synptr->ptrtyp[i] == HYPOPTR
			...
			} else if (ptrtyp == ANTPTR && dbase != ADJ
					&& synptr->pto[i] != 0) {
				sprintf(tbuf, " (Sense %d)\n",
						cursyn->wnsns[synptr->pto[i] - 1]);
				printsynset(prefix, cursyn, tbuf, DEFOFF, synptr->pto[i],
...
static void tracenomins(SynsetPtr synptr, int dbase) {
	...
	for (i = 0; i < synptr->ptrcount; i++) {
		if ((synptr->ptrtyp[i] == DERIVATION)
			...
			sprintf(prefix, "RELATED TO->(%s) ", partnames[synptr->ppos[i]]);
...
int findexample(SynsetPtr synptr) {
	...
	if (vidxfilefp != NULL) {
		...
		sprintf(tbuf, "%s%%%-1.1d:%-2.2d:%-2.2d::", synptr->words[wdnum],
				getpos(synptr->pos), synptr->fnum, synptr->lexid[wdnum]);
...
static void freq_word(IndexPtr index) {
	...
	static char *freqcats[] = { "extremely rare", "very rare", "rare",
			"uncommon", "common", "familiar", "very familiar",
			"extremely familiar" };
		...
		while ((idx = getindex(searchstr, dbase)) != NULL) {
			...
			for (sense = 0; sense < idx->off_cnt; sense++) {
				...
				if (whichsense == ALLSENSES || whichsense == sense + 1) {
					...
					if (skipit != 1) {
						...
						cursyn = read_synset(dbase, idx->offset[sense],
								idx->wd);
						switch (ptrtyp) {
						case ANTPTR:
							if (dbase == ADJ)
			...
			if (whichsense == ALLSENSES) {
				...
				if (i == idx->off_cnt && i == 1)
					...
					sprintf(tmpbuf, "\n%d of %d senses of %s", i, idx->off_cnt,
							idx->wd);
...
static void WNOverview(char *searchstr, int pos) {
	...
	while ((idx = getindex(cpstring, pos)) != NULL) {
		...
		for (sense = 0; sense < idx->off_cnt; sense++) {
			...
			if (!skipit) {
				...
				if (idx->tagged_cnt != -1 && ((sense + 1) <= idx->tagged_cnt)) {
					sprintf(tmpbuf, "%d. (%d) ", sense + 1,
							GetTagcnt(idx, sense + 1));
				} else {
		...
		if (i > 0) {
			if (i == 1)
				sprintf(tmpbuf, "\nThe %s %s has 1 sense", partnames[pos],
						idx->wd);
			else
				sprintf(tmpbuf, "\nThe %s %s has %d senses", partnames[pos],
						idx->wd, i);
			if (idx->tagged_cnt > 0)
...
unsigned int is_defined(char *searchstr, int dbase) {
	...
	while ((index = getindex(searchstr, dbase)) != NULL) {
		...
		retval |= bit(SIMPTR) | bit(FREQ) | bit(SYNS) | bit(WNGREP)
				| bit(OVERVIEW);
...
static int depthcheck(int depth, SynsetPtr synptr) {
	if (depth >= MAXDEPTH) {
		sprintf(msgbuf, "WordNet library error: Error Cycle detected\n   %s\n",
				synptr->words[0]);
		display_message(msgbuf);
...

static void printsynset(char *head, SynsetPtr synptr, char *tail,
...
static void printantsynset(SynsetPtr synptr, char *tail, int anttype,
		int definition) {
	int i, wdcnt;
	char tbuf[SMLINEBUF];
	char *str;
...
static void catword(char *buf, SynsetPtr synptr, int wdnum, int adjmarker,
		int antflag) {
	static char vs[] = " (vs. %s)";
	static char *markers[] = { "", /* UNKNOWN_MARKER */
...
static char *printant(int dbase, SynsetPtr synptr, int wdnum, char *template,
		char *tail) {
	int i, j, wdoff;
	SynsetPtr psynptr;
	char tbuf[WORDBUF];
	...
	for (i = 0; i < synptr->ptrcount; i++) {
		if (synptr->ptrtyp[i] == ANTPTR && synptr->pfrm[i] == wdnum) {
			...
			for (j = 0; j < psynptr->ptrcount; j++) {
				if (psynptr->ptrtyp[j] == ANTPTR && psynptr->pto[j] == wdnum
