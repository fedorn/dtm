...
int getpos(char *s) {
	...
	case 'r':
		return (ADV);
	default:
		sprintf(msgbuf, "WordNet library error: unknown part of speech %s\n",
				s);
		display_message(msgbuf);
		exit(-1);
	}
...
SynsetPtr GetSynsetForSense(char *sensekey) {
	...
	if ((offset = GetDataOffset(sensekey)))
		return (read_synset(GetPOS(sensekey), offset, GetWORD(sensekey)));
	else
		return (NULL);
}
...
char *WNSnsToStr(IndexPtr idx, int sense) {
	...
	if (sstype == SATELLITE)
		sprintf(sensekey, "%s%%%-1.1d:%-2.2d:%-2.2d:%s:%-2.2d", idx->wd,
				SATELLITE, sptr->fnum, sptr->lexid[j], sptr->headword,
				sptr->headsense);
	else
		sprintf(sensekey, "%s%%%-1.1d:%-2.2d:%-2.2d::", idx->wd, pos,
				sptr->fnum, sptr->lexid[j]);
