...
DEFINE_int32(lda_sequence_min_iter, 1, "The maximum number of iterations.");

DEFINE_int32(forward_window, 1,
		"The forward window for deltas. " "If negative, we use a beta with mean " "5.");

DEFINE_string(normalize_docs, "normalize",
		"Describes how documents's wordcounts " "are considered for finding influence. " "Options are \"normalize\", \"none\", " "\"occurrence\", \"log\", or \"log_norm\".");

DEFINE_int32(save_time, 1e20, "Save a specific time.  If -1, save all times.");

DEFINE_int32(fix_topics, 0,
		"Fix a set of this many topics. This amounts " "to fixing these topics' variance at 1e-10.");
...
inf_var* inf_var_alloc(int number_topics, corpus_seq_t* corpus_seq) {
	...
	inf_var_ptr->doc_weights = (gsl_matrix**) malloc(
			sizeof(gsl_matrix*) * corpus_seq->len);
	...
	for (i = 0; i < corpus_seq->len; ++i) {
		...
		if (corpus->ndocs == 0) {
			inf_var_ptr->doc_weights[i] = (gsl_matrix*) malloc(
					sizeof(gsl_matrix));
			inf_var_ptr->doc_weights[i]->size1 = 0;
			inf_var_ptr->doc_weights[i]->size2 = number_topics;
			inf_var_ptr->renormalized_doc_weights[i] = (gsl_matrix*) malloc(
					sizeof(gsl_matrix));
			...
			inf_var_ptr->doc_weights[i] = gsl_matrix_calloc(corpus->ndocs,
					number_topics);
			inf_var_ptr->renormalized_doc_weights[i] = gsl_matrix_calloc(
...
void Solve(gsl_matrix* A, const gsl_vector* b, gsl_vector* x) {
...
void InfluenceTotalFixed(lda_seq* seq, const corpus_seq_t* data) {
	...
	for (int k = 0; k < seq->ntopics; ++k) {
		...
		for (int s = 0; s < seq->nseq; ++s) {
			...
			for (int t = 0; t <= s; ++t) {
				gsl_vector_view w_phi_l = gsl_matrix_column(
						seq->topic[k]->w_phi_l, t);
...
void DumpTimeDocTopicStats(const char* root, size_t t, corpus_t* corpus,
		gsl_matrix** phi) {
...
void PrepareRegressionComponents(corpus_t* corpus, lda_seq* seq, unsigned int k,
		...
		gsl_vector* response, gsl_vector* d_tmp, gsl_matrix* dd_tmp,
		gsl_vector* document_weights, gsl_matrix* W_phi_tmp,
		gsl_vector* exp_h_tmp) {

	// Then left-multiply this by W_phi:
	gsl_blas_dgemv(CblasTrans, 1.0, W_phi, response, 0.0, d_tmp);
	...
	gsl_blas_dgemm(CblasTrans, CblasNoTrans, 1.0, W_phi_tmp, W_phi, 0.0,
			dd_tmp);
...
void SetExpHTmp(lda_seq* seq, const corpus_seq_t* data, unsigned int t,
		unsigned int k, unsigned int n, gsl_vector* exp_h_tmp,
		gsl_vector* zw_tmp, gsl_vector** exp_i_tmp) {
	gsl_vector_set_zero(exp_h_tmp);
	for (int i = t; i < seq->nseq; ++i) {
		gsl_vector_view mean_i_current = gsl_matrix_column(
				seq->topic[k]->e_log_prob, i);
		gsl_vector_view var_i_current = gsl_matrix_column(
				seq->topic[k]->variance, i + 1);
		...
		gsl_vector_scale(zw_tmp,
				scaled_influence[i - t] * scaled_influence[i - t]);
...
double update_inf_var_fixed(lda_seq* seq, const corpus_seq_t* data,
		gsl_matrix** phi, size_t t, const char* root, int dump_doc_stats) {
	double lhood = 0.0;
	...
	gsl_matrix* renormalized_documents_topics_t =
			influence->renormalized_doc_weights[t];

	size_t k;

	gsl_matrix* W_phi = gsl_matrix_calloc(seq->nterms, corpus->ndocs);
	gsl_matrix* W2_phi2 = gsl_matrix_calloc(seq->nterms, corpus->ndocs);
	gsl_matrix* W_phi_var = gsl_matrix_calloc(seq->nterms, corpus->ndocs);
	gsl_matrix* W_phi_tmp = gsl_matrix_alloc(seq->nterms, corpus->ndocs);
	gsl_matrix* dd_tmp = gsl_matrix_alloc(corpus->ndocs, corpus->ndocs);
	gsl_vector* xd_tmp = gsl_vector_calloc(corpus->ndocs);
	gsl_vector* yw_tmp = gsl_vector_calloc(seq->nterms);
	gsl_vector* zw_tmp = gsl_vector_calloc(seq->nterms);
	gsl_vector* terms_inc_tmp = gsl_vector_calloc(seq->nterms);
	gsl_vector** exp_i_tmp = new
	gsl_vector*[seq->nseq];
	...
	double* renormalization_totals = (double*) malloc(
			sizeof(double) * corpus->ndocs);
	for (k = 0; k < documents_topics_t->size2; ++k) {
		...
		for (int d = 0; d < corpus->ndocs; ++d) {
			...
			for (n = 0; n < doc->nterms; ++n) {
				...
				if (FLAGS_normalize_docs == "normalize") {
					...
					renormalization_totals[d] += log(
							doc->count[n] / total_terms[d]);
				} else if (FLAGS_normalize_docs == "identity") {
				...
				gsl_matrix_set(W2_phi2, doc->word[n], d,
						number_terms * number_terms * phi_d_n_k * phi_d_n_k);
		...
		gsl_vector_view w_phi_l_t = gsl_matrix_column(seq->topic[k]->w_phi_l,
				t);
		gsl_vector_set_zero(response);
		for (int i = t; i < seq->nseq - 1; ++i) {
			gsl_vector_view total_influence_time_i = gsl_matrix_column(
					seq->influence_sum_lgl[k], i);
			...
			gsl_vector_view mean_i_current = gsl_matrix_column(
					seq->topic[k]->e_log_prob, i);
			gsl_vector_view mean_i_next = gsl_matrix_column(
					seq->topic[k]->e_log_prob, i + 1);
			gsl_vector_view var_i_current = gsl_matrix_column(
					seq->topic[k]->variance, i + 1);
		...
		PrepareRegressionComponents(corpus, seq, k, W_phi, W_phi_var,
				d_phi_var_tmp, response, d_tmp, dd_tmp,
				&document_weights.vector, W_phi_tmp, exp_h_tmp);
		// Finally, solve for the document weights d!
		Solve(dd_tmp, d_tmp, &document_weights.vector);
		...
		if (FLAGS_save_time == t) {
			...
			sprintf(name, "%s%ld_%ld_weighted_document_terms_var.dat", root, k,
					t);
			f = fopen(name, "w");
			...
			sprintf(name, "%s%ld_%ld_weighted_document_terms_sq.dat", root, k,
					t);
			f = fopen(name, "w");
			...
			if (dump_doc_stats || dump_count % 4 == 0) {
				sprintf(name, "%s%ld_%ld_document_document_matrix.dat", root, k,
						t);
				f = fopen(name, "w");
		...
		gsl_blas_dgemv(CblasNoTrans, 1.0, W_phi, &document_weights.vector, 0.0,
				&w_phi_l.vector);
		...
		gsl_blas_dgemv(CblasNoTrans, 1.0, W2_phi2, xd_tmp, 0.0, yw_tmp);
		gsl_vector_add(terms_inc_tmp, yw_tmp);

		// Store an update coefficient for the beta updates.
		for (int i = t; i < seq->nseq; ++i) {
			gsl_vector_view m_update_coeff_h = gsl_matrix_column(
					seq->topic[k]->m_update_coeff, i);
			if (t == 0) {
		...
		for (int i = t; i < seq->nseq; ++i) {
			gsl_vector_view m_update_coeff_g = gsl_matrix_column(
					seq->topic[k]->m_update_coeff_g, i);
			if (t == 0) {
		...
		for (int i = 0; i < corpus->ndocs; ++i) {
			...
			lhood += (-(value * value + FLAGS_sigma_l * FLAGS_sigma_l)
					/ (2.0 * FLAGS_sigma_d * FLAGS_sigma_d) - 0.5 * log(2 * PI)
...
void make_lda_from_seq_slice(lda* lda_m, lda_seq* lda_seq_m, int time) {
	...
	for (k = 0; k < lda_seq_m->ntopics; k++) {
		// get topic
		gsl_vector s =
				gsl_matrix_column(lda_seq_m->topic[k]->e_log_prob, time).vector;
...
static gsl_matrix* g_alloc(lda_seq* model, const corpus_seq_t* data, int time) {
	gsl_matrix* g = gsl_matrix_calloc(model->nterms, model->ntopics);
	...
	for (int k = 0; k < model->ntopics; ++k) {
		for (int w = 0; w < model->nterms; ++w) {
			...
			gsl_matrix_set(g, w, k,
					(scaled_influence[0] * -variance_first * exp_m));
			...
			for (int i = time; i < model->nseq - 1; ++i) {
				...
				for (int j = 0; j < i; ++j) {
					exp_m = exp(
							-mget(model->topic[k]->e_log_prob, w, j)
									+ mget(model->topic[k]->variance, w, j)
											/ 2.0);
					// Note that we skip the other docs in this time period.
					// Those get special treatment below. 
					if (j != time) {
						influence_other_times += (mget(model->topic[k]->w_phi_l,
								w, j) * scaled_influence[i - j] * exp_m);
					}
				}
...
static gsl_matrix* g3_alloc(lda_seq* model, const corpus_seq_t* data, int time) {
	gsl_matrix* g = gsl_matrix_calloc(model->nterms, model->ntopics);
	...
	for (int k = 0; k < model->ntopics; ++k) {
		for (int w = 0; w < model->nterms; ++w) {
			...
			for (int i = time; i < model->nseq - 1; ++i) {
				...
				total += (scaled_influence[i - time] * exp_m
						* (m_next - m + variance));
			}
...
static gsl_matrix* g4_alloc(lda_seq* model, const corpus_seq_t* data, int time) {
	gsl_matrix* g = gsl_matrix_calloc(model->nterms, model->ntopics);
...
static gsl_matrix* g5_alloc(lda_seq* model, const corpus_seq_t* data, int time) {
	gsl_matrix* g = gsl_matrix_calloc(model->nterms, model->ntopics);
...
static void InferDTMSeq(const int K, unsigned int iter, unsigned int last_iter,
		const corpus_seq_t* data, gsl_matrix* gammas, gsl_matrix* lhoods,
		lda* lda_model, lda_post* post, lda_seq* model, gsl_matrix** suffstats,
		double* bound) {
	int doc_index = 0;
	for (int t = 0; t < data->len; t++) {
		...
		for (int d = 0; d < ndocs; d++) {
			...
			if (iter == 0) {
				doc_lhood = fit_lda_post(d, t, post, NULL,
				NULL,
				NULL,
				NULL,
				NULL);
...
static void InferDIMSeq(const int K, unsigned int iter, unsigned int last_iter,
		...
		gsl_matrix* lhoods, lda* lda_model, lda_post* post, lda_seq* model,
		gsl_matrix** suffstats, double* bound) {
	int doc_index = 0;
	for (int t = 0; t < data->len; t++) {
		// Prepare coefficients for the phi updates.  This change is
		// relatively painless.
		...
		for (int d = 0; d < ndocs; d++) {
			...
			if (iter == 0) {
				...
				doc_lhood = fit_lda_post(d, t, post, model, g, g3_matrix,
						g4_matrix, g5_matrix);
			}
			if (suffstats != NULL) {
				update_lda_seq_ss(t, data->corpus[t]->doc[d], post, suffstats);
			...
			gsl_matrix_view phi_view = gsl_matrix_submatrix(post->phi, 0, 0,
					post->doc->nterms, K);
		...
		if (t < data->len - 1) {
			if (FLAGS_model == "fixed") {
				double l_bound = update_inf_var_fixed(model, data,
...
double lda_seq_infer(lda_seq* model, const corpus_seq_t* data,
		gsl_matrix** suffstats, gsl_matrix* gammas, gsl_matrix* lhoods,
		unsigned int iter, unsigned int last_iter, const char* file_root) {
	int K = model->ntopics;
	int W = model->nterms;
	double bound = 0.0;
	...
	if (FLAGS_model == "fixed") {
		...
		InferDIMSeq(K, iter, last_iter, file_root, data, gammas, lhoods,
				lda_model, &post, model, suffstats, &bound);
	} else if (FLAGS_model == "dtm") {
		InferDTMSeq(K, iter, last_iter, data, gammas, lhoods, lda_model, &post,
				model, suffstats, &bound);
	} else {
...
double fit_lda_seq(lda_seq* m, const corpus_seq_t* data,
	...
	while (iter < FLAGS_lda_sequence_min_iter
			|| ((final_iters_flag == 0 || convergence > LDA_SEQ_EM_THRESH)
					&& iter <= FLAGS_lda_sequence_max_iter) && !last_iter) {
		...
		bound = lda_seq_infer(m, data, topic_suffstats, gammas, lhoods, iter,
				last_iter, file_root);
		if (heldout != NULL) {
			heldout_bound = lda_seq_infer(m, heldout,
			NULL, heldout_gammas, heldout_lhoods, iter, last_iter, file_root);
		}
...
lda_seq* read_lda_seq(const char* root, corpus_seq_t* data) {
	...
	for (int k = 0; k < model->ntopics; k++) {
		...
		model->topic[k]->w_phi_sum = gsl_matrix_alloc(model->nterms,
				model->nseq);
		model->topic[k]->w_phi_l_sq = gsl_matrix_alloc(model->nterms,
				model->nseq);

		if (FLAGS_model == "dim" || FLAGS_model == "regression") {
	...
	if (FLAGS_model == "dim" || FLAGS_model == "regression" && data != NULL) {
		model->influence = (inf_var*) malloc(sizeof(inf_var));
		model->influence->doc_weights = (gsl_matrix**) malloc(
				sizeof(gsl_matrix*));
		int t;
		...
		for (t = 0; t < model->nseq; ++t) {
			...
			model->influence->doc_weights[t] = gsl_matrix_alloc(
					data->corpus[t]->ndocs, model->ntopics);
...
void update_lda_seq_ss(int time, const doc_t* doc, const lda_post* post,
		gsl_matrix** ss) {
...
double fit_lda_seq_topics(lda_seq* model, gsl_matrix** ss) {
...
void init_lda_seq_from_ss(lda_seq* model, double topic_chain_variance,
		double topic_obs_variance, double alpha, gsl_matrix* init_suffstats) {
	gsl_vector_set_all(model->alpha, alpha);

	for (int k = 0; k < model->ntopics; k++) {
		gsl_vector slice = gsl_matrix_column(init_suffstats, k).vector;
		sslm_counts_init(model->topic[k], topic_obs_variance,
				topic_chain_variance, &slice);
		if (k < FLAGS_fix_topics) {
			model->topic[k]->chain_variance = 1e-10;
		}
