...
DEFINE_double(lambda_convergence, 0.01,
		"Specifies the level of convergence required for " "lambda in the phi updates.");
...
double fit_lda_post(int doc_number, int time, lda_post* p, lda_seq* var,
		gsl_matrix* g, gsl_matrix* g3_matrix, gsl_matrix* g4_matrix,
	...
	if (FLAGS_model == "fixed" && var && var->influence) {
		...
		// use!
		topic_view = gsl_matrix_row(var->influence->doc_weights[time],
				doc_number);
	...
	do {
		...
		if (FLAGS_model == "fixed" && var != NULL) {
			update_phi_fixed(doc_number, time, p, var, g3_matrix, g4_matrix,
					g5_matrix);
...
void update_phi_fixed(int doc_number, int time, lda_post* p, lda_seq* var,
	...
	if (var && var->influence) {
		document_weights = gsl_matrix_row(var->influence->doc_weights[time],
				doc_number);
	}
	...
	for (n = 0; n < N; ++n) {
		...
		for (k = 0; k < K; ++k) {
			...
			double w_phi_sum = gsl_matrix_get(var->topic[k]->w_phi_sum, w,
					time);
			...
			term2 = (g3 * term_weight * doc_weight / chain_variance);
			term3 = (term_weight * doc_weight * g4 / chain_variance);
			term4 = (term_weight * term_weight
					* (phi_last * (doc_weight * doc_weight)
							- (doc_weight * doc_weight
									+ FLAGS_sigma_l * FLAGS_sigma_l)) * g5
					/ chain_variance);
...
double compute_lda_lhood(lda_post* p) {
	...
	double lhood = gsl_sf_lngamma(sum(p->model->alpha))
	...
	for (k = 0; k < K; k++) {
		if (p->doc_weight != NULL) {
			...
			if (FLAGS_model == "dim" || FLAGS_model == "fixed") {
				influence_term = -((influence_topic * influence_topic
						+ FLAGS_sigma_l * FLAGS_sigma_l) / 2.0
		...
		double lhood_term = (vget(p->model->alpha, k) - vget(p->gamma, k))
				* e_log_theta_k + gsl_sf_lngamma(vget(p->gamma, k))
				- gsl_sf_lngamma(vget(p->model->alpha, k));

		for (n = 0; n < N; n++) {
			if (mget(p->phi, n, k) > 0) {
				lhood_term += p->doc->count[n] * mget(p->phi, n, k)
						* (e_log_theta_k
		...
		if (ss != NULL)
			for (k = 0; k < model->ntopics; k++)
				for (n = 0; n < p->doc->nterms; n++)
					minc(ss->topics_ss, p->doc->word[n], k,
