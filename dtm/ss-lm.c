	...
	for (t = 0; t < var->T; t++) {
		for (w = 0; w < W; w++) {
			mset(var->e_log_prob, w, t,
					mget(var->mean, w, t + 1) - log(vget(var->zeta, t)));
		}
	}
}
...
void compute_post_mean(int word, sslm_var* var, double chain_variance) {
	...
	for (t = 1; t < T + 1; t++) {
		...
		assert(
				fabs(
						vget(&fwd_variance, t - 1) + chain_variance
								+ var->obs_variance) > 0.0);
		w = var->obs_variance
				/ (vget(&fwd_variance, t - 1) + chain_variance
						+ var->obs_variance);
		vset(&fwd_mean, t,
		...
		if (isnan(vget(&fwd_mean, t))) {
			outlog("t %d word %d w %f %f %f", t, word, w,
					vget(&fwd_mean, t - 1), vget(&obs, t - 1));
	...
	for (t = T - 1; t >= 0; t--) {
		...
		if (isnan(vget(&mean, t))) {
			outlog("t %d w %f %f %f", t, w, vget(&fwd_mean, t),
					vget(&mean, t + 1));
		}
...
void compute_post_variance(int word, sslm_var* var, double chain_variance) {
	...
	for (t = 1; t < T + 1; t++) {
		...
		vset(&fwd_variance, t,
				w * (vget(&fwd_variance, t - 1) + chain_variance));
	}
...
void compute_mean_deriv(int word, int time, sslm_var* var, gsl_vector* deriv) {
	...
	for (t = 1; t <= T; t++) {
		...
		if (var->obs_variance > 0.0) {
			w = var->obs_variance
					/ (vget(&fwd_variance, t - 1) + var->chain_variance
							+ var->obs_variance);
		} else {
			w = 0.0;
		}
...
void compute_obs_deriv(int word, gsl_vector* word_counts, gsl_vector* totals,
	...
	for (t = 0; t < T; t++) {
		...
		for (u = 1; u <= T; u++) {
			...
			term2 += (vget(word_counts, u - 1)
					- (vget(totals, u - 1) * vget(var->T_vct, u - 1)
							/ vget(var->zeta, u - 1))) * dmean_u;

			if (FLAGS_model == "fixed" && u >= FLAGS_forward_window
					&& var->chain_variance > 0.0) {
				double dmean_u_window_prev = vget(&mean_deriv,
						u - FLAGS_forward_window);
				double var_u_window_prev = vget(&variance,
						u - FLAGS_forward_window);
				double mean_u_window_prev = vget(&mean,
						u - FLAGS_forward_window);

				term3 +=
						(exp(
								-(mean_u_window_prev
										- 0.0 * vget(var->zeta, u - 1))
										+ var_u_window_prev / 2.0)
								* ((-mean_u + mean_u_window_prev
										- var_u_window_prev - 1)
										* (dmean_u_window_prev) + (dmean_u))
								* vget(&w_phi_l, u - 1) / var->chain_variance);
				term4 += (exp(
						-2.0
								* (mean_u_window_prev
										- 0.0 * vget(var->zeta, u - 1))
								+ 2.0 * var_u_window_prev)
						* (vget(&m_update_coeff, u - 1)) / var->chain_variance
						* dmean_u_window_prev);
			}
		}
...
void compute_obs_deriv_fixed(int word, gsl_vector* word_counts,
		gsl_vector* totals, sslm_var* var, gsl_matrix* mean_deriv_mtx,
		gsl_vector* deriv) {
	int t, u, T = var->T;
	...
	gsl_vector m_update_coeff_g =
			gsl_matrix_row(var->m_update_coeff_g, word).vector;
	gsl_vector m_update_coeff_h =
			gsl_matrix_row(var->m_update_coeff, word).vector;
				...
				term4 += (exp(
						-2.0
								* (mean_u_window_prev
										- 0.0 * vget(var->zeta, u - 1))
								+ 2.0 * var_u_window_prev)
						* (vget(&m_update_coeff_h, u - 1)) / var->chain_variance
						* dmean_u_window_prev);
			}
		}
		if (var->chain_variance) {
...
void compute_obs_deriv_multiple(int word, gsl_vector* word_counts,
		gsl_vector* totals, sslm_var* var, gsl_matrix* mean_deriv_mtx,
		gsl_vector* deriv) {
	int t, u, T = var->T;
	...
	for (int i = 0; i < T; ++i) {
		gsl_vector_set(g, i,
				exp(-vget(&mean, i) + vget(&variance, i) / 2.0)
						* vget(&w_phi_l, i));
		gsl_vector_set(h, i,
				exp(-2.0 * vget(&mean, i) + 2.0 * vget(&variance, i))
	...
	for (t = 0; t < T; t++) {
		...
		for (u = 1; u <= T; u++) {
			...
			for (int i = 0; i <= u - 1; ++i) {
				...
				term1b += (scaled_influence[u - i - 1] * vget(g, u - i - 1)
			...
			term3 -= (scaled_influence[0] * vget(g, u - 1) * var_u_prev
					* dmean_u_prev);

			// Compute the term involving h(t) and g(t).
			for (int i = 0; i <= u - 1; ++i) {
				term4 += (vget(&mean_deriv, u - i - 1) * scaled_influence[i]
						* scaled_influence[i]
						* (vget(h, u - i - 1)
								- vget(g, u - i - 1) * vget(g, u - i - 1)));
			}
		}
		term1 = -term1 / var->chain_variance;
		term1 = term1
		...
		vset(deriv, t,
				(term1 + term2 + term3 / var->chain_variance
...
double compute_bound(gsl_matrix* word_counts, gsl_vector* totals, sslm_var* var) {
	...
	for (w = 0; w < W; w++) {
		val += (mget(var->variance, w, 0) - mget(var->variance, w, T)) / 2
				* chain_variance;
	}
...
double compute_bound_fixed(gsl_matrix* word_counts, gsl_vector* totals,
		sslm_var* var) {
	int t, T = var->T, w, W = var->W;
	...
	for (t = 1; t <= T; t++) {
		...
		for (w = 0; w < W; w++) {
			...
			term1 += pow(m - prev_m, 2) / (2.0 * chain_variance)
					- v / chain_variance - log(chain_variance);

			term4 += (-(m - prev_m + v) * m_update_coeff_g * exp_i
					/ (2.0 * chain_variance));
...
double compute_bound_multiple(gsl_matrix* word_counts, gsl_vector* totals,
		sslm_var* var) {
	int t, T = var->T, w, W = var->W;
	double term1 = 0, term2 = 0, term3 = 0;
	...
	for (t = 1; t <= T; t++) {
		...
		for (w = 0; w < W; w++) {
			...
			term1 += -pow(delta, 2) / (2 * chain_variance) + v / chain_variance
					+ log(chain_variance);
...
double f_obs(const gsl_vector *x, void *params) {
	...
	for (t = 1; t <= T; t++) {
		...
		if (FLAGS_model == "fixed" && t >= FLAGS_forward_window
				&& p->var->chain_variance > 0.0) {
			double mean_t_window_prev = vget(&mean, t - FLAGS_forward_window);
			double var_t_window_prev = vget(&variance,
					t - FLAGS_forward_window);
			term3 += (exp(
					-(mean_t_window_prev - 0.0 * vget(p->var->zeta, t - 1))
							+ var_t_window_prev / 2.0)
					* (mean_t - mean_t_window_prev + var_t_window_prev)
					* vget(&w_phi_l, t - 1) / p->var->chain_variance);
...
double f_obs_multiple(const gsl_vector *x, void *params) {
	...
	double result = -(term1 + term2 + term3 / p->var->chain_variance
...
void df_obs(const gsl_vector *x, void *params, gsl_vector *df) {
	...
	if (FLAGS_model == "fixed") {
		compute_obs_deriv_fixed(p->word, p->word_counts, p->totals, p->var,
				p->mean_deriv_mtx, df);
	} else if (FLAGS_model == "dtm") {
		compute_obs_deriv(p->word, p->word_counts, p->totals, p->var,
				p->mean_deriv_mtx, df);
	} else {
...
double fit_sslm(sslm_var* var, gsl_matrix* counts) {
	...
	while ((converged > SSLM_FIT_THRESHOLD) && (iter < SSLM_MAX_ITER)) {
		...
		outlog("(%02d) sslm bound = % 10.5f; conv = % 10.5e", iter, bound,
				converged);
	}
