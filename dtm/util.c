...
void * util_malloc(int size) {
	...
	if (p == NULL)
		error("UTIL_MALLOC: Ran out of space. Space in use: %d (%d pointers)\n",
	...
	*((int *) p) = size;
	if (display_allocs)
		fprintf(stderr,
				"UTIL_MALLOC: Allocated %d bytes, %d bytes total, %d pointers\n",
...
void * util_calloc(int num, int size) {
	...
	if (p == NULL)
		error("UTIL_CALLOC: Ran out of space. Space in use: %d (%d pointers)\n",
				space_in_use, pointers_in_use);
	...
	*((int *) p) = num * size;
	if (display_allocs)
		fprintf(stderr,
				"UTIL_CALLOC: Allocated %d bytes, %d bytes total, %d pointers\n",
...
void * util_realloc(void * p, int size) {
	...
	if (realp == NULL)
		error(
				"UTIL_REALLOC: Ran out of space. Space in use: %d (%d pointers)\n",
				space_in_use, pointers_in_use);
	*((int *) (realp)) = size;
	space_in_use += (size - oldsize);
	if (display_allocs)
		fprintf(stderr,
				"UTIL_REALLOC: Allocated %d bytes, %d bytes total, %d pointers\n",
				size, space_in_use, pointers_in_use);
...
void util_free(void * p) {
	...
	if (display_allocs)
		fprintf(stderr,
				"UTIL_FREE: Freed up %d bytes, %d bytes remaining, %d pointers\n",
				size, space_in_use, pointers_in_use);
}
