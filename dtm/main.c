...
DEFINE_string(mode, "fit",
		"The function to perform. " "Can be fit, est, or time.");
DEFINE_string(model, "dtm", "The function to perform. " "Can be dtm or dim.");
DEFINE_string(corpus_prefix, "",
		"The function to perform. " "Can be dtm or dim.");
DEFINE_string(lda_model_prefix, "",
		"The name of a fit model to be " "used for testing likelihood.  Appending \"info.dat\" " "to this should give the name of the file.");
DEFINE_int32(heldout_time, -1,
		"A time up to (but not including) which we wish to train, " "and at which we wish to test.");
DEFINE_string(output_table, "", "");
DEFINE_string(params_file, "settings.txt",
...
void fit_dtm(int min_time, int max_time) {
	...
	if (max_time >= 0) {
		...
		for (int i = min_time; i < max_time; ++i) {
			...
			max_nterms =
					max_nterms > corpus->nterms ? max_nterms : corpus->nterms;
	...
	lda_seq* model_seq = new_lda_seq(data_subset, data_subset->nterms,
	...
	double* table = (double*) malloc(
			sizeof(double) * data_full->corpus[max_time]->ndocs);
...
int main(int argc, char* argv[]) {
	...
	if (FLAGS_mode == "time") {
		...
		corpus_seq_t* data = read_corpus_seq(
		...
		for (int t = 0; t < data->len; t++) {
			table[t] = (double*) malloc(
					sizeof(double) * data->corpus[t]->ndocs);
