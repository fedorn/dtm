...
DEFINE_double(sigma_l, 0.05, "If true, use the new phi calculation.");
DEFINE_double(sigma_d, 0.05, "If true, use the new phi calculation.");
DEFINE_double(sigma_c, 0.05, "c stdev.");
DEFINE_double(sigma_cv, 1e-6, "Variational c stdev.");
DEFINE_double(resolution, 1,
...
DEFINE_double(time_resolution, 0.5,
		"This is the number of years per time slice.");
DEFINE_double(influence_mean_years, 20.0,
		"How many years is the mean number of citations?");
DEFINE_double(influence_stdev_years, 15.0,
		"How many years is the stdev number of citations?");
DEFINE_int32(influence_flat_years, -1,
		"How many years is the influence nonzero?" "If nonpositive, a lognormal distribution is used.");
...
corpus_seq_t* make_corpus_seq_subset(corpus_seq_t* all, int start, int end) {
	...
	subset_corpus->corpus = (corpus_t**) malloc(
			sizeof(corpus_t*) * subset_corpus->len);
	for (n = start; n < end; n++) {
...
corpus_t* read_corpus(const char* name) {
	...
	if (fileptr == NULL) {
		outlog("Error reading corpus prefix %s. Failing.", filename);
		exit(1);
	}
	c->ndocs = 0;
	c->nterms = 0;
	...
	while ((fscanf(fileptr, "%10d", &length) != EOF)) {
		...
		c->doc[c->ndocs]->log_likelihoods = (double*) malloc(
...
corpus_seq_t* read_corpus_seq(const char* name) {
	...
	corpus_seq->corpus = (corpus_t**) malloc(
			sizeof(corpus_t*) * corpus_seq->len);
	// allocate corpora
	int doc_idx = 0;
	int ndocs, i, j;
	corpus_seq->ndocs = 0;
	for (i = 0; i < corpus_seq->len; ++i) {
		...
		for (j = 0; j < ndocs; j++) {
			if (doc_idx >= raw_corpus->ndocs) {
				outlog(
						"Error: too few documents listed in dtm sequence file %s.\n"
								"Current  line: %d %d %d.\n", filename, doc_idx,
						ndocs, j);
				exit(1);
			}
...
double* NewScaledInfluence(int size) {
	...
	for (int i = 0; i < size; ++i) {
		...
		double x = (i / FLAGS_time_resolution)
				+ (1.0 / FLAGS_time_resolution) / 2;
