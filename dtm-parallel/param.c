...
void param_init_par() {
	...
	param_init();
	if ((ddpar == NULL) || (ddpar->par_inited))
		return;
		...
		while ((c = getc(ddinf->stream)) != EOF) {
			...
			case '/':
				if ((state == QUOTATION) || (state == DQUOTATION))
					cclass = OTHER;
				else if ((next_c = getc(ddinf->stream)) == '*')
			...
			case BEGIN_LINE:
				...
				case COMMENT_START:
					...
					break;
				case COMMENT_END:
					ddinf_error("Unmatched comment ending");
				case DELIMITER:
					ddinf_error("Missing parameter name.");
				case QUOTE:
					ddinf_error("Quotations for values only.");
				case DQUOTE:
					ddinf_error("Quotations for values only.");
				case ENDMARK:
					state = ENDMARKED;
			...
			case NAME:
				...
				case COMMENT_END:
					ddinf_error("Unmatched comment ending");
				case QUOTE:
					ddinf_error("Quotes for values only.");
				case DQUOTE:
					ddinf_error("Quotes for values only.");
				case DELIMITER:
					state = VALUE;
			...
			case VALUE:
				...
				case COMMENT_END:
					ddinf_error("Unmatched comment ending");
				case DELIMITER:
					ddinf_error("Missing delimiter or endmark.");
				case ENDMARK:
			...
			case DIRECTIVE:
				...
				case WHITESPACE:
					...
					break;
				case OTHER:
					command_buffer[commandBufferIndex++] = c;
					break;
				default:
					ddinf_error("Illegal character in ddinf directive");
				}
				break;
			case ARGUMENT:
				switch (cclass) {
				...
				case ENDMARK:
					...
					if (strcmp(command_buffer, "include") == 0) {
						...
						ddinf_error("Unrecognized ddinf directive.");
					break;
				case END_OF_LINE:
					ddinf_error("Missing end marker");
				default:
			...
			case QUOTATION:
				if (cclass == QUOTE)
					state = QUOTATION_ENDED;
				else
					ddvalue_buffer[valueBufferIndex++] = c;
				break;
			case DQUOTATION:
				if (cclass == DQUOTE)
					state = DQUOTATION_ENDED;
				else
					ddvalue_buffer[valueBufferIndex++] = c;
				break;
			case QUOTATION_ENDED:
				...
				case COMMENT_START:
					...
					break;
				case COMMENT_END:
					ddinf_error("Unmatched comment end.");
				case ENDMARK:
					ddname_buffer[nameBufferIndex++] = EOS;
					ddvalue_buffer[valueBufferIndex++] = EOS;
...
ddlink *insert(const char *newddname, const char *newddvalue_raw) {
	...
	if ((lp = lookup(newddname)) == NULL) {
		...
		if (lp == NULL || lp->ddname == NULL || lp->ddvalue == NULL)
			return NULL;
...
int param_geti(const char *var_name, int dflt) {
	...
	if ((lp = prefixed_lookup(var_name)) == NULL)
		...
		return safe_atol(var_name, lp->ddvalue);
}
...
int param_symvarie(const char *var_name, int *var) {
	...
	if ((lp = prefixed_lookup(var_name)) == NULL)
		...
		*var = safe_atol(var_name, lp->ddvalue);
	return TRUE;
}
...
double param_getf(const char *var_name, double dflt) {
	...
	if ((lp = prefixed_lookup(var_name)) == NULL)
		...
		return safe_atod(var_name, lp->ddvalue);
}
...
int param_symvarfe(const char *var_name, double *var) {
	...
	if ((lp = prefixed_lookup(var_name)) == NULL)
		...
		*var = safe_atod(var_name, lp->ddvalue);
	return TRUE;
}
...
int param_getb(const char *var_name, int dflt) {
	...
	if ((lp = prefixed_lookup(var_name)) == NULL)
		...
		for (i = 0; i < NumberOfYesStrings; i++)
...
long safe_atol(const char *name, const char *expr) {
	...
	if (strcmp(suffix, "") != 0)
		error("PARAM: Unable to convert value \"%s\" for ddname \"%s\"", expr,
				name);
	return d;
}
...
int param_push_file(const char *fn) {
	...
	if (strcmp(DDINF_DEFAULT_NAME, fn) == 0)
		pname = ddinfName;

	for (e = ddpar; e != NULL; e = e->next)
		if (strcmp(e->file_name, pname) == 0)
			break;
...
char * param_pop_file(void) {
	...
	if ((d != NULL) && (d->next == NULL))
		return NULL; /* Don't allow the stack to be */
	else if (ddpar != NULL)
		ddpar = ddpar->next; /* completely cleared          */
