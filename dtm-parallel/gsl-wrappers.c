...
void mtx_fscanf(const char* filename, gsl_matrix * m) {
	...
	outlog("reading %ld x %ld matrix from %s", m->size1, m->size2, filename);
	if (!fileptr) {
		outlog("Error opening file %s. Failing.", filename);
		exit(1);
	}
...
void mtx_fprintf(const char* filename, const gsl_matrix * m) {
	outlog("writing %ld x %ld matrix to %s", m->size1, m->size2, filename);
	FILE* fileptr;
	fileptr = fopen(filename, "w");
	if (!fileptr) {
...
void choose_k_from_n(int k, int n, int* result) {
	...
	gsl_ran_choose(RANDOM_NUMBER_GENERATOR, (void *) result, k, (void *) x, n,
			sizeof(int));
}
...
//        gsl_multimin_fdfminimizer_vector_bfgs;
	...
	gsl_multimin_fdfminimizer * opt = gsl_multimin_fdfminimizer_alloc(method,
			dim);
...
void optimize_f(int dim, gsl_vector* x, void* params,
	...
	gsl_multimin_fminimizer * opt = gsl_multimin_fminimizer_alloc(method, dim);
